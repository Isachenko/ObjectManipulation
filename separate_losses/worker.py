...
class Worker():
    ...
    def train(self, rollout, sess, gamma, bootstrap_value):
        ...
        #advantages = discounted_rewards + gamma * self.value_plus[1:] - self.value_plus[:-1]
        ...
        v_l, p_l, e_l, g_n_v, g_n_p, v_n_v, v_n_p, self.batch_rnn_state, _, _ = sess.run([self.local_AC.value_loss,
                                                                     self.local_AC.policy_loss,
                                                                     self.local_AC.entropy,
                                                                     self.local_AC.grad_norms_value,
                                                                     self.local_AC.grad_norms_policy,
                                                                     self.local_AC.var_norms_value,
                                                                     self.local_AC.var_norms_policy,
                                                                     self.local_AC.state_out,
                                                                     self.local_AC.apply_grads_value,
                                                                     self.local_AC.apply_grads_policy],
                                                                    feed_dict=feed_dict)
        return v_l / len(rollout), p_l / len(rollout), e_l / len(rollout), g_n_v, g_n_p, v_n_v, v_n_p
            ...
            while not coord.should_stop():
                ...
                while self.env.is_episode_finished() == False:
                    ...
                    if len(episode_buffer) == 30 and d != True and episode_step_count != max_episode_length - 1:
                        ...
                        v_l, p_l, e_l, g_n_v, g_n_p, v_n_v, v_n_p = self.train(episode_buffer, sess, gamma, v1)
                        episode_buffer = []
                        sess.run(self.update_local_ops)
                    if d == True:
                        break
                ...
                if len(episode_buffer) != 0:
                    v_l, p_l, e_l, g_n_v, g_n_p, v_n_v, v_n_p = self.train(episode_buffer, sess, gamma, 0.0)
                ...
                if episode_count % STATISTICS_SAVE_TIME_STEP == 0 and episode_count != 0:
                    ...
                    summary.value.add(tag='Losses/Grad Norm value', simple_value=float(g_n_v))
                    summary.value.add(tag='Losses/Grad Norm policy', simple_value=float(g_n_p))
                    summary.value.add(tag='Losses/Var Norm value', simple_value=float(v_n_v))
                    summary.value.add(tag='Losses/Var Norm policy', simple_value=float(v_n_p))
